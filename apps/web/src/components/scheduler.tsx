  ...
  return (
    ...
    >
      <ComboboxTrigger
        ...
        render={<Button className="w-24 font-normal tabular-nums" size="sm" variant="outline" />}
      >
        <ComboboxValue />
      </ComboboxTrigger>
  ...
  return (
    <div className="divide-y">
      {days.map((day) => {
        ...
        const startIndex = daySchedule ? Math.floor(daySchedule.startMinute / 15) : 0;
        const endIndex = daySchedule ? Math.floor(daySchedule.endMinute / 15) : 0;
        ...
        return (
          ...
          >
            <Label className="flex h-8 w-30 shrink-0 items-center gap-2.5">
              <Switch
                ...
                onCheckedChange={(checked) =>
                  updateDay(day.index, checked ? { startMinute: 9 * 60, endMinute: 17 * 60 } : null)
                }
              />
              {day.label}
            </Label>
            ...
            {daySchedule ? (
              <Group aria-label={`${day.label} schedule`} className="w-fit">
                <TimeCombobox
                  ariaLabel={`${day.label} start time`}
                  items={timeOptions.slice(0, -1)}
                  value={timeOptions[startIndex]!}
                  onChange={(start) => {
                    const startMinute = timeOptions.indexOf(start) * 15;
                    const endMinute =
                      startMinute >= daySchedule.endMinute
                        ? Math.min(startMinute + 60, 23 * 60 + 45)
                        : daySchedule.endMinute;
                    updateDay(day.index, { startMinute, endMinute });
                  }}
                />
                <GroupSeparator />
                ...
                <TimeCombobox
                  ...
                  onChange={(end) =>
                    updateDay(day.index, {
                      ...daySchedule,
                      endMinute: timeOptions.indexOf(end) * 15,
                    })
                  }
                />
              </Group>
            ) : (
              <p className="flex h-8 items-center text-sm text-muted-foreground">Not scheduled</p>
            )}
          </div>
        );
      })}
