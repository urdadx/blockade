...
export function RedirectPage({
  ...
  useEffect(() => {
    ...
    setRemainingSeconds((mode === "focus" ? sessionDuration : breakDuration) * 60);
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      if (nextSeconds === 0 && !hasCompletedRef.current) {
        ...
        setIsRunning(false);
        setMode(nextMode);
        setRemainingSeconds((nextMode === "focus" ? sessionDuration : breakDuration) * 60);
      }
    };
    update();
    const interval = window.setInterval(update, 250);
    return () => window.clearInterval(interval);
  }, [isRunning, mode, sessionDuration, breakDuration]);
  ...
  const toggleTimer = () => {
    ...
    const nextSeconds = remainingSeconds > 0 ? remainingSeconds : modeDuration * 60;
    setRemainingSeconds(nextSeconds);
    ...
    <main className="relative grid min-h-svh w-full place-items-center overflow-hidden bg-black p-4 sm:p-6">
      ...
      <section className="relative z-10 w-full max-w-5xl mx-auto justify-center rounded-lg border border-white/15 bg-black/55 p-5 text-white shadow-2xl backdrop-blur-md sm:p-8 lg:p-10">
          ...
          {showPomodoroTimer && (
            <div className="relative z-10 mx-auto max-w-full overflow-x-auto py-2">
              ...
              <FlipClock
                ...
                showDays="never"
                size="md"
                variant={mode === "focus" ? "outline" : "secondary"}
        ...
        <div className="mt-6 flex justify-between  pt-4">
          ...
          >
            <GithubStar />
            <a href="https://github.com/urdadx/blockade" target="_blank" rel="noopener noreferrer">
              Star us on GitHub
            </a>
          </Button>
        </div>
