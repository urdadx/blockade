...
import { getDomainCategoryIds } from "@blockade/core";
...
export function TopCategoriesChart({ data }: { data: { domain: string; attempts: number }[] }) {
  const groupedData = Object.entries(
    data.reduce<Record<string, number>>((totals, { domain, attempts }) => {
      if (attempts <= 0) return totals;
      const categories = getDomainCategoryIds(domain).filter(isConfiguredCategory);
      if (categories.length === 0) {
        totals.other = (totals.other ?? 0) + attempts;
        return totals;
      }
      for (const category of categories) {
        totals[category] = (totals[category] ?? 0) + attempts;
      }
      return totals;
    }, {}),
      ...
      {groupedData.length > 0 ? (
        <div className="flex flex-col items-center gap-4 sm:flex-row">
          <div className="h-52 w-full min-w-0 sm:w-1/2">
            ...
            >
              <Tooltip variant="frosted-glass" valueFormatter={(value) => `${value} times`} />
