...
export default function FlipClock({
  ...
  }, [countdown, targetDate, timeSeconds]);

  const displayedTime = timeSeconds === undefined ? time : getTimeFromSeconds(timeSeconds);

  const daysStr = String(displayedTime.days).padStart(3, "0");
  const hoursStr = String(displayedTime.hours).padStart(2, "0");
  const minutesStr = String(displayedTime.minutes).padStart(2, "0");
  const secondsStr = String(displayedTime.seconds).padStart(2, "0");

  const shouldShowDays =
    countdown && (showDays === "always" || (showDays === "auto" && displayedTime.days > 0));
  ...
  return (
    ...
    >
      <span className="sr-only absolute">
        {`${displayedTime.hours}:${displayedTime.minutes}:${displayedTime.seconds}`}
      </span>
...
function getTimeFromSeconds(value: number): TimeLeft {
